...
<body>
	...
	<c:forEach var="dahoon" items="${ dahoonList }">
		<section id="article">
			<h2><span id="navi-profile"><img src="<c:url value="/resources/img/dahoon2.png" />"/></span>${ dahoon.title }</h2>
			${ dahoon.contents }
			<p></br><a href="javascript:delArticle(${ dahoon.id })"><i class="icon-trash" style="opacity:0.5;float:right;"></i></a></p>
		</section>
	</c:forEach>
	
	
	<div id="type_text" class="modal hide fade" style="display: none; width:550px; padding:30px; ">
		<form name="text_write" action="/spicy/dahoon" method="POST">
	...
	$(window).ready(function() {
		$('#post-btn').click(function(event) {
			...
			$.ajax({
				type : 'POST',
				url : 'http://localhost:8080/spicy/dahoon',
				dataType : "json", 
	...
	function delArticle(id) {
		...
		$.ajax({
			type : 'POST',
			url : 'http://localhost:8080/spicy/dahoon/'+id,
			dataType : "json", 
			data: { _method:"DELETE"},
			success : function(data) {
				if(!data.result) {
