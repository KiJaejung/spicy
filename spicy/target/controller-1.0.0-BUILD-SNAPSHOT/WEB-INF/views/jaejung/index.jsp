...
<body>
	...
	<c:forEach var="jaejung" items="${ jaejungList }">
		<section id="article">
			<h2><span id="navi-profile"><img src="<c:url value="/resources/img/jaejung.png" />"/></span>${ jaejung.title }</h2>
			${ jaejung.contents }
			<p></br><a href="javascript:delArticle(${ jaejung.id })"><i class="icon-trash" style="opacity:0.5;float:right;"></i></a></p>
		</section>
	</c:forEach>
	
	
	<div id="type_text" class="modal hide fade" style="display: none; width:550px; padding:30px; ">
		<form name="text_write" action="/spicy/jaejung" method="POST">
	...
	$(window).ready(function() {
		$('#post-btn').click(function(event) {
			...
			$.ajax({
				type : 'POST',
				url : 'http://localhost:8080/spicy/jaejung',
				dataType : "json", 
	...
	function delArticle(id) {
		...
		$.ajax({
			type : 'POST',
			url : 'http://localhost:8080/spicy/jaejung/'+id,
			dataType : "json", 
			data: { _method:"DELETE"},
			success : function(data) {
				if(!data.result) {
